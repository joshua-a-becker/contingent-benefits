...
d_valid = d %>%
  ...
  mutate(
    ...
    , seth_toward_truth = ifelse((pre_influence < mu1 & mu1 > truth) | (pre_influence > mu1 & mu1 < truth), T, F)
    , eq = pre_influence==mu1
  )
...
aggreg =  d_valid %>%
  ...
  summarize(
    ...
    , varadj = var/mu1
    , skew = skewness(pre_influence)
    , kurtosis = kurtosis(pre_influence)
    , varman = mean((pre_influence-mean(pre_influence))^2)
    , varmanadj = sum(((pre_influence-mean(pre_influence))^2))/(length(pre_influence)-1)
